...
entity acquisition_stsa_top_CTRL_s_axi is
generic (
...
end entity acquisition_stsa_top_CTRL_s_axi;
...
architecture behave of acquisition_stsa_top_CTRL_s_axi is
    ...
    constant ADDR_PHASE_OUT_DATA_0      : INTEGER := 16#20#;
    constant ADDR_PHASE_OUT_CTRL        : INTEGER := 16#24#;
    constant ADDR_PEAK_OUT_DATA_0       : INTEGER := 16#30#;
    constant ADDR_PEAK_OUT_DATA_1       : INTEGER := 16#34#;
    constant ADDR_PEAK_OUT_CTRL         : INTEGER := 16#38#;
    constant ADDR_DETECTED_OUT_DATA_0   : INTEGER := 16#48#;
    constant ADDR_DETECTED_OUT_CTRL     : INTEGER := 16#4c#;
    constant ADDR_MAX_POWER_OUT_DATA_0  : INTEGER := 16#58#;
    constant ADDR_MAX_POWER_OUT_CTRL    : INTEGER := 16#5c#;
    constant ADDR_MEAN_POWER_OUT_DATA_0 : INTEGER := 16#68#;
    constant ADDR_MEAN_POWER_OUT_CTRL   : INTEGER := 16#6c#;
    ...
    signal int_peak_out_ap_vld : STD_LOGIC;
    signal int_peak_out        : UNSIGNED(47 downto 0) := (others => '0');
    signal int_detected_out_ap_vld : STD_LOGIC;
    signal int_detected_out    : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_PEAK_OUT_DATA_0 =>
                        rdata_data <= RESIZE(int_peak_out(31 downto 0), 32);
                    when ADDR_PEAK_OUT_DATA_1 =>
                        rdata_data <= RESIZE(int_peak_out(47 downto 32), 32);
                    when ADDR_PEAK_OUT_CTRL =>
                        rdata_data(0) <= int_peak_out_ap_vld;
                    when ADDR_DETECTED_OUT_DATA_0 =>
                        rdata_data <= RESIZE(int_detected_out(0 downto 0), 32);
                    when ADDR_DETECTED_OUT_CTRL =>
                        rdata_data(0) <= int_detected_out_ap_vld;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_peak_out <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (peak_out_ap_vld = '1') then
                    int_peak_out <= UNSIGNED(peak_out);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_peak_out_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (peak_out_ap_vld = '1') then
                    int_peak_out_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_PEAK_OUT_CTRL) then
                    int_peak_out_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_detected_out <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (detected_out_ap_vld = '1') then
                    int_detected_out <= UNSIGNED(detected_out);
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_detected_out_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (detected_out_ap_vld = '1') then
                    int_detected_out_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_DETECTED_OUT_CTRL) then
                    int_detected_out_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
